...
build_dev_rpi() {
    ...
    cp -rf ${SOURCE_DIR}/build/ext/yiot-core/yiot/common/iotkit/modules/crypto/converters/libconverters.so ${SOURCE_DIR}/build/dist
...
build_app_linux() {
    ...
    make deploy
    popd
    cp -f ${SOURCE_DIR}/build/ext/yiot-core/yiot/common/iotkit/modules/crypto/converters/libconverters.so ${SOURCE_DIR}/build/yiot.dist/lib
    if [ "${BUILD_PKG}" == "1" ]; then
...
build_emulator_linux() {
    ...
    cp -f ${SOURCE_DIR}/build/ext/yiot-core/yiot/common/iotkit/modules/crypto/converters/libconverters.so ${SOURCE_DIR}/build/yiot.dist/
    if [ "${BUILD_PKG}" == "1" ]; then
      find_tool mock
      if [ "${FIND_RES}" == "1" ]; then
        print_message "Build RPM package skipping"
