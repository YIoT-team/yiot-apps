...
_initializer_exec_task(void *pvParameters) {
    ...
    int i = 0;
    // const vs_snap_service_t *snap_cfg_server = NULL;
    // const vs_snap_service_t *snap_info_server = NULL;
    // const vs_snap_service_t *snap_prvs_server = NULL;
...
//         VS_IOT_MEMSET(&device_type, 0, sizeof(device_type));
//         // VS_IOT_MEMSET(&serial, 0, sizeof(serial));

//         uint8_t *p = (uint8_t *)&device_type;
//         p[0] = 1;
...
//         uint32_t device_roles = (uint32_t)VS_SNAP_DEV_THING | (uint32_t)VS_SNAP_DEV_THING;
